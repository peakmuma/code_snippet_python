...
the_url='http://cl.bearhk.info/'
external_url=the_url+'thread0806.php?fid=22&search=&page={0}'
external_list=[]
...
def buildHtml(dataList,path,start=0,end=100):
        soup=BeautifulSoup('<html><head><meta http-equiv="Content-Type" content="text/html;charset=utf-8"></head><body><ul></ul></body></html>')
        ul=soup.ul
        for data in dataList[start:end]:
                data['url']=the_url+data['url']
                li=soup.new_tag('li')
                a=soup.new_tag('a',href=data['url'],target='_blank')
                a.string=data['title']
                li.append(a)
                li.append(str(data['maxCommentNum']))
                ul.append(li)
